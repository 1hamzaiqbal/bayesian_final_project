...
def fit_gp(X, y, kernel=None, noise_level=0.001, normalize_y=False, n_restarts=8):
...
def bayesian_optimization_with_acq(X_pool, y_pool, acquisition_fn, acq_params,
    ...
    for iteration in range(n_iterations):
        ...
        gp = fit_gp(X_train, y_train_transformed, kernel=kernel, noise_level=noise_level)
...
def run_acquisition_comparison(n_runs=20, save_dir=None):
    ...
    noise_levels = [0.001, 0.1, 1, 5]
    acquisition_order = list(acquisition_configs.keys())
    all_results = {}
    
    for dataset_idx, (name, (X_pool, y_pool, f_opt, use_log, kernel)) in enumerate(datasets.items()):
        print(f"\n{'='*50}")
        print(f"  {name} DATASET")
        print(f"{'='*50}")
        
        n_pool = len(X_pool)
        dataset_noise_results = {}
        
        # Pre-generate all init_indices for this dataset (shared across methods & noise levels)
        all_init_indices = []
        ...
        for noise_idx, noise_std in enumerate(noise_levels):
            print(f"\n  Observation noise stdev = {noise_std:.3f}")
            
            # Pre-generate noisy observation pools per run (shared across acquisitions)
            noisy_pools = []
            base_seed = dataset_idx * 10000 + noise_idx * 1000 + 7
            for run in range(n_runs):
                if noise_std > 0:
                    rng_noise = np.random.default_rng(seed=base_seed + run)
                    noise = rng_noise.normal(0.0, noise_std, size=y_pool.shape)
                else:
                    noise = np.zeros_like(y_pool)
                y_noisy = y_pool + noise
                if use_log:
                    # Ensure log(y + 1) remains valid by keeping y > -1
                    y_noisy = np.maximum(y_noisy, -0.999 + 1e-6)
                noisy_pools.append(y_noisy)
            
            noise_results = {}
            
            for acq_name, acq_params in acquisition_configs.items():
                print(f"    Testing {acq_name}...")
                
                runs = []
                for run in range(n_runs):
                    init_indices = all_init_indices[run].copy()
                    y_pool_noisy = noisy_pools[run]
                    
                    if acq_name == 'Random':
                        rs_rng = np.random.default_rng(seed=run * 42 + noise_idx * 100 + 1000)
                        history = random_search(
                            X_pool, y_pool_noisy, n_initial=5, n_iterations=30,
                        ...
                        effective_noise = max(noise_std, 1e-6)
                        history = bayesian_optimization_with_acq(
                            X_pool, y_pool_noisy, acq_fn, acq_params,
                            n_initial=5, n_iterations=30,
                ...
                noise_results[acq_name] = runs
                
                final_gaps = []
                for run_history in runs:
                    f_best_initial = run_history['best_so_far'][4]
                    f_best_found = run_history['best_so_far'][-1]
            ...
            dataset_noise_results[noise_std] = noise_results
        
        all_results[name] = {
            'results': dataset_noise_results,
            'f_opt': f_opt,
            'noise_levels': noise_levels
        }
    
    print("\n✓ All runs verified: All methods share identical initial points per run")
    return all_results, acquisition_order


def plot_comparison_results(all_results, save_dir, acquisition_order):
    """Create learning-curve style plots for acquisition functions."""
    print("\n" + "=" * 70)
    print("CREATING LEARNING CURVE PLOTS")
    print("=" * 70)
    
    dataset_items = list(all_results.items())
    ...
    noise_levels = dataset_items[0][1]['noise_levels']
    n_rows = len(noise_levels)
    n_cols = len(dataset_items)
    ...
    for col, (dataset_name, data) in enumerate(dataset_items):
        f_opt = data['f_opt']
        for row, noise_std in enumerate(noise_levels):
            ax = get_ax(row, col)
            dataset_results = data['results'][noise_std]
            
            for acq_name in acquisition_order:
                runs = dataset_results.get(acq_name, [])
                if not runs:
                    continue
                ...
                mean_gap = gaps.mean(axis=0)[4:]
                se_gap = gaps.std(axis=0)[4:] / np.sqrt(len(runs))
            ...
            if col == 0:
                ax.set_ylabel(f'Gap (noise_std={noise_std:.4f})')
    ...
    plt.suptitle('Learning Curves: Acquisition Functions across Noise Levels', fontsize=13)
...
def plot_mean_gap_summary(all_results, save_dir, acquisition_order):
    """Plot grouped bar charts of mean gap per acquisition with bars per noise level."""
    dataset_items = list(all_results.items())
    if not dataset_items:
        return
    ...
    width = 0.7 / len(noise_levels)
    color_map = plt.cm.viridis(np.linspace(0.3, 0.9, len(noise_levels)))
    
    for col, (dataset_name, data) in enumerate(dataset_items):
        ax = axes[col]
        f_opt = data['f_opt']
        x = np.arange(len(acquisition_order))
        
        for idx, noise_std in enumerate(noise_levels):
            runs_by_acq = data['results'][noise_std]
            means = []
            for acq_name in acquisition_order:
                runs = runs_by_acq.get(acq_name, [])
                final_gaps = []
                for run in runs:
                    f_best_initial = run['best_so_far'][4]
                    f_best_found = run['best_so_far'][-1]
                    final_gaps.append(compute_gap(f_best_found, f_best_initial, f_opt))
                means.append(np.mean(final_gaps) if final_gaps else 0.0)
            
            offsets = x + (idx - (len(noise_levels) - 1) / 2) * width
            ax.bar(offsets, means, width=width, color=color_map[idx],
                   label=f"noise_stdev={noise_std:.2f}")
        
        ax.set_xticks(x)
        ax.set_xticklabels(acquisition_order, rotation=20, ha='right', fontsize=9)
        ax.set_ylim([0, 1.05])
        ax.set_title(f'{dataset_name}')
        ax.set_ylabel('Mean Gap' if col == 0 else '')
        ax.grid(True, axis='y', alpha=0.3)
    
    handles = [
        plt.Line2D([0], [0], marker='s', linestyle='',
                   color=color_map[idx], label=f"noise std={noise_levels[idx]:.4f}",
                   markersize=10)
        for idx in range(len(noise_levels))
    ]
    fig.legend(handles=handles, loc='upper center', ncol=len(noise_levels),
               bbox_to_anchor=(0.5, 1.05))
    plt.suptitle('Mean Final Gap by Acquisition Function and Noise Level', fontsize=13, y=1.08)
    plt.tight_layout(rect=[0, 0, 1, 0.98])
    plt.savefig(os.path.join(save_dir, 'acquisition_mean_gap.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print("Saved: acquisition_mean_gap.png")
...
def compute_statistics_and_rankings(all_results, save_dir):
    ...
    for dataset_name, data in all_results.items():
        ...
        for noise_std in data['noise_levels']:
            print(f"\n  Observation noise σ = {noise_std:.3f}")
            stats_list = []
            dataset_results = data['results'][noise_std]
            n_runs = None
            
            for acq_name, runs in dataset_results.items():
                final_gaps = []
                for run in runs:
                    f_best_initial = run['best_so_far'][4]
                    f_best_found = run['best_so_far'][-1]
            ...
            print(f"    Rankings (n={n_runs} runs):")
            ...
            summary_stats[dataset_name][noise_std] = stats_list
...
def main():
    ...
    # Run main comparison
    all_results, acquisition_order = run_acquisition_comparison(n_runs=5, save_dir=save_dir)
    
    # Create comparison plots
    plot_comparison_results(all_results, save_dir, acquisition_order)
    plot_mean_gap_summary(all_results, save_dir, acquisition_order)
    ...
    print("\nBest acquisition function by dataset & noise level:")
    for dataset, noise_stats in summary_stats.items():
        for noise_std, stats in noise_stats.items():
            best = stats[0]
            print(f"  {dataset} (noise std={noise_std:.2f}): {best['name']} (gap = {best['mean']:.4f})")
    
    print("\nFiles generated:")
    print("  - acquisition_comparison.png")
    print("  - acquisition_mean_gap.png")
